...
class Hash_Set:
    ...
    def _rehash(self):
        ...
        # Update the capacity by the specific change factor from the notes
        change_factor = 2 * self._capacity + 1
        self._capacity = change_factor

        # Save the old table and create a new empty table
        ...
        while len(old_table) > 0:
            old_slot = old_table.pop(0)

            while len(old_slot) > 0:
                value = old_slot.pop(0)
                new_slot = self._find_slot(value)
                new_slot.insert(0, value)
