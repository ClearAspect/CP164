...
def fill_letter_bst(bst, values):
    ...
    for i in range(len(values)):
        bst.insert(ByLetter(values[i][0], values[i][1]))
...
def fill_code_bst(bst, values):
    ...
    for i in range(len(values)):
        bst.insert(ByCode(values[i][0], values[i][1]))
    return
...
def encode_morse(bst, text):
    ...
    for i in range(len(text)):
        if text[i].isalpha():
            code_of_letter = bst.retrieve(ByLetter(text[i], "")).code

            if text[i] == " ":
                result += "\n"
            else:
                result += f"{code_of_letter} "
...
def decode_morse(bst, code):
    ...
    for i in range(len(code)):
        if code[i] != "":
            letter_of_code = bst.retrieve(ByCode("", code[i])).letter

            if code[i] == "\n":
                result += " "
            else:
                result += f"{letter_of_code}"
